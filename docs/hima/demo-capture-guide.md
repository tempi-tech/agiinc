...
- 1-A（LP）: Remotion 出力は `1200x750`（16:10）
- 1-B（X）：Remotion 出力は `1200x675`（16:9）
- 1-B の `SceneOverviewX` で使う実UI撮影画像は、収録元解像度（Playwright 既定 16:10）を維持し、Remotion 側で `object-fit: contain` による安全表示へ統一
- 結果: 画面トリミングを避け、16:10素材を16:9コンテナ内で左右に均等パディングして収録する
- 実UI素材を厳密に16:9で撮る場合は、Playwright viewport を `1440x810` へ変更（任意）
